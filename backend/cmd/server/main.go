...
func (h *Handler) HealthCheck(c *gin.Context) {
	...
	if !exists {
		// Если логгера нет в контексте, используем дефолтный
		h.logger.Info("Health check requested (no request logger)", nil)
	} else {
		requestLogger := loggerInterface.(*logger.Logger)
...
func (h *Handler) CreateItem(c *gin.Context) {
	...
	loggerInterface, exists := c.Get("logger")
	if !exists {
		h.logger.Warn("No logger in context", nil)
